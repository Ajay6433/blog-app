...
const Hero = () => {
  ...
  let data = [
    {
      ...
      "paragraph": "Welcome to our community of bloggers, where your words have the power to connect, inspire, and transform. Here, we provide you with a safe space to share your experiences, reflections, and knowledge with an enthusiastic audience."
    },
    {
      "id": 2,
      "image": "/community.png",
  ...
  return (
    <div className='w-full z-20 mt-3 px-4 md:px-8 lg:px-16'>
      {data.map((item) => (
        <div key={item.id} className="p-6 flex flex-col md:flex-row justify-center items-center gap-5 bg-white rounded-lg mb-6">
          <Image src={item.image} alt={item.title} width={100} height={100} className="mb-4 md:mb-0" />
          <div>
            <h2 className="text-2xl md:text-3xl font-semibold mb-2">{item.title}</h2>
            <p className="text-gray-600 text-[14px] md:text-[16px] text-justify">{item.paragraph}</p>
          </div>
        </div>
      ))}
    </div>
